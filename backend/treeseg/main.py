...
from baselines import EquiSeg, RandomSeg
...
parser.add_argument(
    ...
    type=int,
    required=False,
    help="Meeting index (zero-indexed) to segment. If this is not set the entire fold will be processed.",
)
parser.add_argument("--eval", action="store_true", help="Evaluation mode")
parser.add_argument(
    "--fold", required=True, choices=["dev", "test"], help="Dataset fold"
)
...
avg_lvl_pk_loss = [
    pk_sum / count if count > 0 else None
    for pk_sum, count in zip(sum_lvl_pk, lvl_count)
]
avg_lvl_wdiff_loss = [
    wdiff_sum / count if count > 0 else None
    for wdiff_sum, count in zip(sum_lvl_wdiff, lvl_count)
]
