...
class GroqProvider(LLMProvider):
    """Groq implementation supporting both sync and async operations"""
    ...
    def _prepare_args(self, messages: List[Message], **kwargs) -> Dict[str, Any]:
        """Prepare arguments for Groq's Chat Completions API (OpenAI-compatible)"""
        ...
        for msg in messages:
            if isinstance(msg["content"], str):
                ...
                # Note: Groq uses OpenAI-compatible format, so we just concatenate the text
                content_text = " ".join(
    ...
    def generate(
        ...
        try:
            ...
            if "internal server error" in error_msg or "500" in error_msg:
                # Let the retry decorator handle 500 errors
